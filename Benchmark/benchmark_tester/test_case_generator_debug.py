...
class EnhancedTestCaseGenerator:
    ...
    def _generate_variations(self, base_question, num_variants=3):
        modifiers = [
            lambda s: s + '?' * random.randint(1, 2),
            lambda s: s.replace(' ', '  '),
        ...
        return [self._apply_modifiers(base_question, modifiers, random.randint(2, 3))
    ...
    def generate_test_suite(self):
        # 목표 : 전체 약 100개의 테스트 케이스 생성
        # 각 유형별 목표 개수를 설정 (예를 들어, 5개 유형당 20개씩 → 총 100개 내외)
        test_suite = {'exact_match': [], 'synonym': [], 'unstructured': [], 'ambiguous': [], 'invalid': []}
        target_total_per_category = 5

        # 모호성 케이스: _generate_high_ambiguity_questions()는 5개 항목을 반환하므로,
        # 각 항목당 20/5 = 4개 케이스 생성
        ambiguous_questions = self._generate_high_ambiguity_questions()
        for q, kws in ambiguous_questions:
            for _ in range(target_total_per_category // len(ambiguous_questions)):
                test_suite['ambiguous'].append((self._add_noise(q), kws))

        # 무효 케이스: 유사하게 5개 케이스에서 각 4개씩 → 총 20개
        invalid_samples = self._generate_tricky_invalid_cases()
        for q, _ in invalid_samples:
            for _ in range(target_total_per_category // len(invalid_samples)):
                test_suite['invalid'].append((self._add_noise(q), None))

        # 정확 매칭 케이스: keyword_map은 4개 항목 있으므로, 각 항목당 20/4 = 5개 케이스 생성
        for kw in self.keyword_map:
            base = f"{self.faker.year()}년 {kw} 일정을 알려주세요"
            variations = self._generate_variations(base)
            count = 0
            for v in variations:
                if count < target_total_per_category // len(self.keyword_map):
                    test_suite['exact_match'].append((self._add_noise(v), kw))
                    count += 1

        # 동의어 케이스: 단순히 20번 반복하여 전체 20개 생성
        for _ in range(target_total_per_category):
        ...
        # 비정형 케이스: unstructured_samples 3개 항목이 있으므로, 각 항목당 20/3 ≒ 6개 케이스 생성 (총 약 18개)
        ...
        for q, kw in unstructured_samples:
            for _ in range(target_total_per_category // len(unstructured_samples)):
                test_suite['unstructured'].append((self._add_noise(q), kw))
